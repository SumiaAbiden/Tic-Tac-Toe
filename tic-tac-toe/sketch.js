...
let player1Score = 0;//to keep track of the score of the first player
let player2Score = 0;//to keep track of the score of the second player
...
// Initializes a 3x3 array with empty elements
let gameFloor = [
  [' ', ' ', ' '],
  [' ', ' ', ' '],
  [' ', ' ', ' ']
...
function checkWinner() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (winningLine(gameFloor[0][i], gameFloor[1][i], gameFloor[2][i])) {
      ...
      winnerPosition = [2, i];//2 stands for drawing vertical line & i for the current column
  ...
  //check TIE sitiuation(No winner & no empty cell) 
    ...
    if (gameFloor[i][j] == ' ') {
      ...
      if (result != null) {
        if (result == 'tie') {
          console.log('Tie!');
          song1.play();
          //Add one to the score of each player
          player1Score++;
          player2Score++; 
        }
          ...
          song2.play(0, 1, 5, 1, song2.duration() / 4);
...
function resetGame() {
  ...
  currentTurn++;
  winnerBoolean = false;//intializing winnerBoolean again to false to start new game
  ...
  for (let i = 0; i < 3; i++) {
   for (let j = 0; j < 3; j++){
        let y = heigh * i + heigh / 2 + 100;//centering the y in the cell vertically
          let x = wid * j + wid / 2;//centering the x in the cell horizontally
